Vagrant.configure(2) do |config|
  ...
  config.vm.define :dockerhost do |config|
    ...
    config.vm.provision "shell", env: {"apt_proxy" => ENV["apt_proxy"]}, inline: <<-EOF
      ...
      if [ $apt_proxy ]; then
        docker build -t logicaldoc/logicaldoc-ce-docker --build-arg APT_PROXY=$apt_proxy .
      else
        docker build -t logicaldoc/logicaldoc-ce-docker .
      fi

      #docker run --rm -v logicaldoc-repo:/opt/logicaldoc/repository -v logicaldoc-conf:/opt/logicaldoc/conf logicaldoc/logicaldoc-ce-docker
      docker run -d --name logicaldocdms --restart=always -p 8080:8080 -v logicaldoc-repo:/opt/logicaldoc/repository -v logicaldoc-conf:/opt/logicaldoc/conf logicaldoc/logicaldoc-ce-docker
    EOF
  end
end
